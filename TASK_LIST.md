...
#### 1.4.1 実装
- [x] **1.4.1.1** AudioEngine クラス
  - [x] 状態管理（IDLE/ARMED/RUNNING/DEGRADED/ERROR）
  - [x] Capture → DSP → Output パイプライン
  - [x] スレッド管理

- [x] **1.4.1.2** Virtual Mic 出力
  - [x] 共有メモリ経由の音声供給
  - [ ] フェードイン/アウト
...
- [~] **1.7.1** pkg インストーラ作成
  - [x] ドライバインストールスクリプト（build_driver.sh, install_driver.sh）
  - [x] アンインストールスクリプト（uninstall_driver.sh）
  - [ ] 署名設定
...
### 即座に開始可能
1. **1.1.3.1** デバイス認識テスト（ドライバビルド・インストール確認）
2. **1.2.1.1** DeviceManager 実装（入出力デバイス列挙）
3. **1.3.1.1** DSPChain フレームワーク（各モジュール実装）

### ブロッカー確認が必要
- Apple Developer Program 登録状況（コード署名・Notarization に必要）
- 開発用 Mac の Xcode 環境確認
